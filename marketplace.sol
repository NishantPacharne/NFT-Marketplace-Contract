...
contract Marketplace is ReentrancyGuard {
    ...
    function resellToken(address nftContract, uint256 tokenId, uint256 price) public payable {
        ...
        require(idToMarketItem[tokenId].isListed == false, "Item already listed.");
    ...
    function removeListing(uint256 tokenId, address nftContract) public payable nonReentrant{
        MarketItem storage item = idToMarketItem[tokenId];
    ...
    function updatePrice(uint256 tokenId, uint256 price) public {
        MarketItem storage item = idToMarketItem[tokenId];
        require(item.isListed == true, "Item is not listed for sale.");
        require(msg.sender == item.seller, "Only item owner can perform this operation");
        item.price = price;
    }
    ...
    function getListedNFTsByUser(address user) public view returns (uint256[] memory) {
        uint256 count = 0;
        for (uint256 i = 0; i < listedTokenIds.length; i++) {
